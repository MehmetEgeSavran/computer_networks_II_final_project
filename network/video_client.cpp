...
int runVideoClient(int socket) {
    ...
    SwsContext* scaling_context = nullptr;
    ...
    while (!glfwWindowShouldClose(custom_window.getWindow()) && !stop) {
#ifdef _WIN32
        u_long avail = 0;
        ioctlsocket(socket, FIONREAD, &avail);
        ...
        if (hasData) {
            uint32_t size_net;
            if (!recvAll(socket, reinterpret_cast<uint8_t*>(&size_net), 4))
                break;
            uint32_t size = ntohl(size_net);
            if (size == 0) {break;}
            std::vector<uint8_t> packet(size);
            if (!recvAll(socket, packet.data(), size)) {break;}
            if (redecoder.decodePacket(packet, decoded)) {
                if (!scaling_context) {
                    scaling_context = sws_getContext(
                ...
                uint8_t* destination_ptr[4] = { rgb_buffer, nullptr, nullptr, nullptr };
    ...
    if (scaling_context) sws_freeContext(scaling_context);
